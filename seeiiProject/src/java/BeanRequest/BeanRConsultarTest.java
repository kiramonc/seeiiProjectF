...
public class BeanRConsultarTest {
    ...
    public List<Tema> verItemsMenu(String username) {
        this.session = null;
        this.transaction = null;
        try {
            DaoTema daoTema = new DaoTema();
            ...
            this.session = HibernateUtil.getSessionFactory().openSession();
            this.transaction = session.beginTransaction();
            List<Tema> temas = daoTema.verPorUnidad(session, daoEstudiante.verPorUsername(session, username).getUnidadensenianza().getId());
            CrearBayesDynamic rbDynamic= new CrearBayesDynamic();
            for (int i = 0; i < temas.size(); i++) {
                if(rbDynamic.getNumPreguntas(temas.get(i).getNombre())==0){
                    temas.remove(i);
                }
            }
